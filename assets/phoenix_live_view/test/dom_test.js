...
describe("DOM", () => {
  ...
  test("isNowTriggerFormExternal", () => {
    ...
    expect(DOM.isNowTriggerFormExternal(form, "phx-trigger-external", document.body)).toBe(true)

    form = tag("form", {}, "")
    document.body.appendChild(form)
    // lvp:modify-next-line
    // expect(DOM.isNowTriggerFormExternal(form, "phx-trigger-external")).toBe(false)
    expect(DOM.isNowTriggerFormExternal(form, "phx-trigger-external", document.body)).toBe(false)
